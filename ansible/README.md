...
- **Kubernetes**:
  ...
  - [`k8s_deploy_postgresql.yml`](./k8s_deploy_postgresql.yml): Deploying a central [PostgreSQL](https://www.postgresql.org/) instance (Bitnami Helm chart) in the `data-services` namespace with a persistent Longhorn volume and metrics, shared by the cluster's applications.
  - [`k8s_deploy_clickhouse.yml`](./k8s_deploy_clickhouse.yml): Deploying a central [ClickHouse](https://clickhouse.com/) instance (Bitnami Helm chart) in the `data-services` namespace with a persistent Longhorn volume and metrics, backing the Plausible analytics events.
  - [`k8s_deploy_redis.yml`](./k8s_deploy_redis.yml): Deploying a central [Redis](https://redis.io/) instance (Bitnami Helm chart) in the `data-services` namespace with a persistent Longhorn volume, authentication and metrics, for applications needing a queue broker or cache.
  - [`k8s_deploy_tailscale.yml`](./k8s_deploy_tailscale.yml): Installing the [Tailscale Kubernetes operator](https://tailscale.com/kb/1236/kubernetes-operator) in the `tailscale` namespace. `k8s_deploy_redis.yml` is what marks the Redis Service for exposure, so this one runs first.
  - [`k8s_deploy_plausible.yml`](./k8s_deploy_plausible.yml): Deploying [Plausible Analytics](https://plausible.io/) against the central PostgreSQL and ClickHouse instances and exposing it through the ingress controller. Requires `k8s_deploy_postgresql.yml` and `k8s_deploy_clickhouse.yml` first.
  - [`k8s_deploy_workloads.yml`](./k8s_deploy_workloads.yml): Deploying an application as any number of Deployments and CronJobs sharing a namespace and the central PostgreSQL and Redis instances, each Deployment optionally exposed by its own Service and Ingress, whereas a CronJob declaring either is rejected. See the [Kubernetes Deployment Guide](../DEPLOYMENT.md#multi-workload-deployments) for the variable file schema.
  ...
  - [`k8s_deploy_pushgateway.yml`](./k8s_deploy_pushgateway.yml): Deploying a [Prometheus Pushgateway](https://github.com/prometheus/pushgateway) in the monitoring namespace as the metrics sink the backup and verification CronJobs report to. Apply after `k8s_setup_backups.yml`.
  - [`k8s_setup_backup_verify.yml`](./k8s_setup_backup_verify.yml): Scheduling CronJobs that continuously prove the backups are restorable by round-tripping the real backup and restore scripts into a scratch database and pushing the result to Pushgateway. Requires the `backup-scripts` ConfigMap from `k8s_setup_backups.yml` and the Pushgateway above.
  - [`k8s_deploy_backup_alerts.yml`](./k8s_deploy_backup_alerts.yml): Applying the Prometheus alert rules for missing, stale, or unrestorable backups (Telegram delivery is configured in `k8s_prometheus_grafana.yml`). See the [Backups & Restore guide](../BACKUPS.md#automated-verification--alerting).
